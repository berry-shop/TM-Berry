    ...
    .then(items => {
      ...
      items.forEach(item => {
        ...
        const productItem = `<div class="catalog__item ${classification}">
          <div class="image-box">
            <img loading="lazy" decoding="async" class="catalog__img" src="${image}" alt="${alt}" title="${alt}">
          </div>
          <div>
          ...
          <div>
            <p class="catalog__flavour" style="display: flex; flex-direction: column;">
              <span class="available" style="margin-bottom: 1px;">${availability}</span>
              <span>${flavour}</span>
            </p>
          </div>
          <div>
          ...
          <div class="description-wrapper">
            <button type="button" style="user-select: none;" aria-label="опис товару" data-link class="catalog__item-btn">опис</button>
            <div class="popup__info" data-popup>
              <div class="close-box">
                <i role="button" tabindex="0" aria-label="закрити вікно" class="fa-solid fa-xmark close-bnt" style="font-size: 26px; line-height: 50%; display: flex; justify-content: flex-end; margin-top: 5px; cursor: pointer;"></i>
              </div>
              <p>${description}</p>
              <div class="special-code__description">Код товару: <span>${code}</span></div>
              <img loading="lazy" decoding="async" class="description__img" src="${image}" alt="${alt}" title="${alt}">
              <div class="popup__bottom">
                <div class="popup__delivery">
                  <ul class="dilvery__list-popup" style="padding: 0; list-style-type: none; margin: 0; margin-top: 10px;">
                    <li class="dilvery__list-item-header" style="font-weight: 500;">Доставка</li>
                ...
                <div class="popup__payment">
                  <ul class="dilvery__list-popup" style="padding: 0; list-style-type: none; margin-top: 10px; margin-bottom: 10px;">
                    <li class="dilvery__list-item-header" style="font-weight: 500;">Оплата</li>
      ...
      available.forEach(el => {
        if (el.textContent === "В наявності ✓") {
          el.style.color = 'var(--accept-color)';
        } else if (el.textContent === "Немає в наявності ✗") {
          el.style.color = 'var(--accent-color-darker)';
        }
      });
      //////////////////////////////////////////////
      // пагинация
      ...
      const catalogLinks = catalogCards.querySelectorAll('.catalog__item-btn');
